...
//!
//! So the sys crate publishes the directory as `cargo:lib_dir` and this
...
//! uppercased — here `DEP_ACADSHARP_NATIVE_LIB_DIR`, which is the key the
//! shipped `acadsharp-rs` publishes too, so a consumer who copies this
//! reads a variable that exists. It is the only way a `-sys` crate can
//! tell its dependents something they have to express as an argument, and
//! this file is here to show it working end to end rather than to be
//! clever: `libviprs-dep` published a mac archive nothing could link (#95)
//! because nothing in the repository ever linked one.
//!
